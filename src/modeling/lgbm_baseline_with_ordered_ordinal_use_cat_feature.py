...
"""
- Summary
    - Baseline with LGBM manually ordered ord_1, ord_2 and then LGBM's default categorical feature handling is used
...
EXP_DETAILS="Baseline with LGBM manually ordered ord_1, ord_2 and then LGBM's default categorical feature handling is used"
...
# Extract the cat codes
# This is only needed for LGBM for cat variable handling.
for name in combined_df.columns:
    combined_df[name] = combined_df[name].cat.codes
...
result_dict = utility.make_prediction_classification(logger, run_id, train_X, train_Y, test_X, features=features.tolist(),
                                                     params=lgb_params, seed=SEED, 
                                                     kf=kf, model_type=MODEL_TYPE, 
                                                     plot_feature_importance=PLOT_FEATURE_IMPORTANCE, cat_features=cat_features.tolist())
